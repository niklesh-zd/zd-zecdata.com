...
use GT3\PhotoVideoGalleryPro\Block\FS_Slider;
use GT3\PhotoVideoGalleryPro\Block\Grid;
use GT3\PhotoVideoGalleryPro\Block\Masonry;
use GT3\PhotoVideoGalleryPro\Block\Packery;
use GT3\PhotoVideoGalleryPro\Block\Slider;
use GT3\PhotoVideoGalleryPro\Block\Thumbnails;
use GT3\PhotoVideoGalleryPro\Settings;
use GT3\PhotoVideoGalleryPro\Rest_Api;
...
if(!class_exists('GT3_Post_Type_Gallery')) {
	...
	class GT3_Post_Type_Gallery {
		...
		private static $galleries = array(
			'grid'       => Grid::class,
			'masonry'    => Masonry::class,
			'packery'    => Packery::class,
			'slider'     => Slider::class,
			'fsslider'   => FS_Slider::class,
...
//				wp_enqueue_style('gt3-cpt-gallery', plugins_url('/static/cpt-gallery.css', __FILE__), array(), self::VERSION);
		...
		public function render_shortcode($atts){
			...
			if(!$isREST && (($post->post_type === self::post_type && $rendered) || doing_action( 'wpseo_head' )/* || !did_action('wp_print_scripts')*/)) {
				return '';
			}
			...
			if($atts['type'] === '' || !is_string($atts['type'])) {
			...
			$gallery_settings = $this->getSettings($atts['type']);
			if (is_array($gallery_settings)) {
				$settings = array_merge($settings, $gallery_settings);
			}
