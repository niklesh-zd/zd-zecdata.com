...
class Assets {
	...
	protected $assets = array(
		...
		'gt3pg_pro--albums'          => array( 'gt3pg_pro--core', 'gt3pg_pro--isotope' ),
		'gt3pg_pro--zoom'            => array( 'imagesloaded' ),
		'gt3pg_pro--carousel'        => array( 'gt3pg_pro--core', 'gt3pg_pro--lightbox' ),
	);
	...
	public static $block_style_depends = array(
		...
		'albums'       => array( 'gt3pg_pro--gallery-isotope', 'gt3pg-pro-blocks-frontend' ),
		'carousel'       => array( 'gt3pg_pro--gallery-isotope', 'gt3pg-pro-blocks-frontend', 'gt3pg_pro--carousel' ),
	);
	...
	private function __construct(){
		$this->optimizer_enabled = defined('GT3PG_FHD_PLUGINENABLED');
...
//		add_action('wp_enqueue_scripts', array( $this, 'frontend_elementor' ));
		add_action('elementor/frontend/after_enqueue_styles', array( $this, 'frontend_elementor' ));
		...
		add_action('admin_print_styles', function(){
			wp_register_style( 'gt3-gallery-pro-admin-styles', false );
			wp_enqueue_style( 'gt3-gallery-pro-admin-styles' );
			$css = '#adminmenu img{ width: 100%; height: 100%; max-width: 20px; max-height: 20px; }';
			wp_add_inline_style( 'gt3-gallery-pro-admin-styles', $css );
		});
	}
...
//			trigger_error('JS file <b>'.self::$js_path.$file.'</b> not found.', E_USER_WARNING);
	...
	public function print_elementor_styles(){
		if(is_singular()) {
			...
			if (false !== $document && method_exists($document, 'get_elements_data')) {
	...
	public static function enqueue_block_script($block){
		if($block instanceof Basic_Block) {
			...
			if(key_exists($name, static::$block_style_depends)) {
				wp_enqueue_script('gt3pg_pro--'.$name);
