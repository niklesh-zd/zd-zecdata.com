...
class Packery extends Isotope_Gallery {
	...
	protected function renderItem($image, &$settings){
		...
		$video = $this->renderVideo($image, $settings);
		if ($video) {
			$render            .= $video;
		}	else {
	...
	protected function render($settings){
		...
		switch($settings['hoverType']) {
			...
			case 'onHover':
				$this->add_render_attribute(
					'_wrapper', 'data-hover-align', 'center_top'
				);

				list($halign, $valign) = explode('_', $settings['hoverTextPosition']);
				$this->add_render_attribute(
					'_wrapper',
					array(
						'class' => array(
						...
						'data-hover-valign' => $valign,
						'data-hover-halign' => $halign,
						'data-hover-type' => $settings['hoverTextAnimation'],
					)
				);

				$this->add_style('.text_info_wrapper span', array(
					'color: %s !important' => $settings['hoverTextColor']
				));

				$this->add_style('.img-wrapper:before', array(
					'background: %s !important' => $settings['hoverBackgroundColor']
				));
...
//				$settings['hover'],
		...
		if($settings['lightbox']) {
			...
			$this->data_settings['lightbox']['options'] = array(
				...
				'toggleControlsOnSlideClick' => $settings['lightboxToggleControlsOnSlideClick'],
				'closeOnSlideClick'          => $settings['lightboxCloseOnSlideClick'],
				'videoAutoplay'              => $settings['lightboxVideoAutoplay'],
				'animationType'              => $settings['lightboxAnimationType'],
				'simpleStyle'              => $settings['lightboxSimpleStyle'],
				'continuous'                 => $settings['lightboxContinuous']
			);

			if($settings['ytWidth']) {
				$this->data_settings['lightbox']['options']['ytWidth'] = true;
			}
		}
		$this->data_settings['packery'] = $settings['packery'];
		$this->add_style(
