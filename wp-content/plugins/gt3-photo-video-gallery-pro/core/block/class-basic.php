...
use GT3\PhotoVideoGalleryPro\Single_Trait;
...
abstract class Basic {
	...
	protected $lightbox_local_index = 0;
	...
	protected function serializeImages(&$settings){
		...
		if(is_array($settings['ids']) && !!count($settings['ids'])) {
			foreach($settings['ids'] as $image_key => &$image) {
				...
				if($_image && in_array($_image['type'], $this->allowed_mime)) {
		...
		list($order, $asc) = explode('_', $settings['customOrder'].'_');
		
		if(key_exists('random', $settings)) {
			if(
				true === $settings['random']
				|| '1' === $settings['random']
				|| 1 === $settings['random']
			) {
				$order = 'random';
			} else if(
				false === $settings['random']
				|| '0' === $settings['random']
				|| 0 === $settings['random']
			) {
				$order = 'ordered';
		...
		if($order) {
			switch($order) {
				...
				case 'alt':
				case 'date':
					$settings['ids'] = $this->array_sort($settings['ids'], $order, !!$asc ? SORT_DESC : SORT_ASC);
					break;
			}
		}
	...
	public function render_block($settings){
		...
		$this->data_settings = apply_filters('gt3pg-pro/gallery/render/data-settings', $this->data_settings);
		$this->data_settings = apply_filters('gt3pg-pro/gallery/render/'.$this->name.'/data-settings', $this->data_settings);
		?>
		<script type="application/json" id="settings--<?php echo $this->_id; ?>"><?php echo wp_json_encode($this->data_settings) ?></script><?php
